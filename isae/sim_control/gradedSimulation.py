...
class gradedSimulation(walkSimulation):
    ...
    def updateGrades(self):
        ...
        #print(self.grades[0])
        # RMS       
        self.grades[1] += self.updateGrade_RMStoQdotRef(np.vstack([.1, 0, 0, 0, 0, 0]), np.vstack([150, 20, 20, 1, 1, 1]), self.dt) + 0.1*self.updateGrade_constantOrientation(np.array([0,0,0,1]))
